					...
					<div class="inner-content form-all-input selec_grade">
						...
						<!--@foreach($org_data as $data)
							<div class="form-group  col-sm-4 select-unit">
								<label class="label_data_{{$data->set_id}}" data-text="Select {{ $data->set_name }}" for="select_level">{{ $data->set_name }} <span class="required">*</span></label>
								<select name="gradeId[{{$data->set_id}}]" class="select2 form-control select_level level_id_{{$data->set_id}}" data-set-id="{{ $data->set_id}}">
