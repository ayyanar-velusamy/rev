(function($) {
   'use strict';
    ...
    $.easyAjax = function(options, callback) {
        var defaults = {
            type: 'GET',
            container: 'body',
            blockUI: true,
        ...
        if (typeof opt.complete != "function") {
            opt.complete = function (jqXHR, textStatus) {
                ...
                if (opt.disableButton) {
					var res1 = JSON.parse(jqXHR.responseText);
					 if (typeof res1.reload == "undefined" && typeof res1.redirect == "undefined") {
        ...
        if (opt.file == true) {
			...
			if(profile_image_data != "" && profile_image_data instanceof Blob){
				data.append('image',profile_image_data);
			}
			opt.data = data;
        }
        $.ajax({
				...
				if (response.status) {
					...
					if (response.action == "redirect") {
                        if (opt.redirect) {
                            ...
                            showResponseMessage(message, "error", {
                                timeOut: 100000,
                                positionClass: "toast-top-right"
                            });
							setTimeout(function(){
    ...
    $.showErrors = function(object) {
        ...
        for (var i = 0; i < keys.length; i++) {
            ...
            helpBlockContainer.append('<span id="'+ keys[i] +'" class="help-block error removeclass-valid">' + object[keys[i]] + '</span>');
            $(grp).addClass("has-error");
        }
    }
})(jQuery);
